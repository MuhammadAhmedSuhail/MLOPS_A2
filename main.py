...
dag = DAG(
    ...
    schedule='@once',  # Run it once
)
def extract_dawn(data_dir):
    print("Starting extract_dawn function...")
    # The URL of the Dawn News website's page you want to scrape
    ...
    if response.status_code == 200:
        ...
        # Find all <a> tags with the class name 'story__link'
        ...
        with open(os.path.join(data_dir, 'dawn_news_articles.csv'), mode='w', newline='', encoding='utf-8') as file:
            ...
            # Iterate over the found <a> tags containing 'https://www.dawn.com' and 'news'
            for link in story_links:
                href = link.get('href')
                title = link.get_text(strip=True)
                print(f"Processing link: {title} - {href}")
                if href and re.search(r'^https://www\.dawn\.com.*news', href):
                    # Find the corresponding story__excerpt div
                    ...
                    if article_response.status_code == 200:
                        ...
                        # Find the first <p> tag containing the article description
                        first_paragraph = article_soup.find('p')
                        
                        # Write the title, URL, description, and excerpt to the CSV file
                        if first_paragraph:
...
def fetch_article_paragraph(url):
    ...
    # Navigate the JSON to find the first paragraph of the article
    try:
...
def extract_bbc(data_dir):
    ...
    # Find divs with the specified data-testid attributes
        ...
        try:
            ...
            if article_link_tag:
                ...
                if full_url.find("live") == -1 and not full_url.startswith("https://cloud.email.bbc.com/") and full_url.find("videos") == -1:
                    ...
                    if excerpt_tag:
                        excerpt = excerpt_tag.get_text(strip=True)  # Get the text content of the <p> tag
                        #print(url+href)
            ...
            continue  # Skip if the div doesn't contain an <a> tag
...
def extract_data(**kwargs):
    ...
    # Create the assignment2 directory
    ...
    # Create the data directory inside assignment2
...
def print_working_directory(**kwargs):
    ...
    return cwd
# Define the PythonOperator to print the working directory
...
# Define the BashOperator to run git and dvc commands
dvc_commands = """
...
git init
dvc init
git config user.email "muhammadahmedsuhail007@gmail.com"
git config user.name "MuhammadAhmedSuhail"
git commit -m "dvc commit"
dvc add data/bbc_articles.csv data/dawn_news_articles.csv
git add data/dawn_news_articles.csv.dvc data/.gitignore data/bbc_articles.csv.dvc
git commit -m "dvc commit 2"
dvc remote add -d storage gdrive://1b8kotSS8Z7f-ywXWVk3wkkMJ0w-lmTks
git commit .dvc/config -m "Configure remote storage"
dvc push
git remote add origin https://MuhammadAhmedSuhail:<token>@github.com/MuhammadAhmedSuhail/MLOPS_A2.git
git branch -M main
